...
def filter_data(codes, asts, nls, paths):
    """
    ...
    for i in range(len(codes)):
        ...
        if len(code) > config.max_code_length or len(nl) > config.max_nl_length or len(nl) < config.min_nl_length:
            continue
        if get_path_max_len(path) < 2:
            continue 
        new_codes.append(code)
        new_asts.append(ast)
        new_nls.append(nl)
        new_paths.append(path)
    return new_codes, new_asts, new_nls, new_paths
...
def pad_one_batch(batch: list, vocab: Vocab) -> torch.Tensor:
    ...
    """
    '''
    exzample:itertools.zip_longest
    res = list(itertools.zip_longest([1,2,3], [5,6], fillvalue=0))
    for item in res:
        print(item)
    # (1, 5), (2, 6), (3,0)
    '''
...
def unsort_collate_fn(batch, code_vocab, ast_vocab, nl_vocab, raw_nl=False):
    ...
    :return: 以一个batch中最长句子的长度作为T， 注意每一个batch的T是不一样的，同一个batch的T是一样的. [T, B]
    """
    batch = batch[0]
    code_batch = []
    ast_batch = []
    ...
    path, path_len, path_statement_len, path_statement_token_len = path_to_indexes(
            path_batch, config.len_path, config.len_path_statement, config.len_path_statement_token,
            code_vocab.word2index)

    return code_batch, code_seq_lens, ast_batch, ast_seq_lens, nl_batch, nl_seq_lens, path, path_len, path_statement_len, path_statement_token_len
...
def measure(batch_size, references, candidates):
    """
...
def meteor_score(reference, candidate):
    ...
    :return:
    """
    reference = set(' '.join(reference))
    hypothesis = set(' '.join(candidate))
...
def read_path_data(path_file_name):
    paths = []
    with open(path_file_name, 'r', encoding='utf-8') as f:
        for line in f:
            json_line = json.loads(line.strip())
            paths.append(json_line)
    return paths

def path_to_indexes(path_batch, len_path, len_path_statement, len_path_statement_token, word2index):
    path_indexes_batch = []
    path_len_batch = []
    path_statement_len_batch = []
    path_token_len_batch = []
    for paths in path_batch:
        path_indexes = []
        path_statement_lens = []
        path_token_lens = []
        for path in paths:
            path_indexed, path_statement_len, path_token_len = path_statements_to_indexes(path, len_path_statement,
                                                                                                len_path_statement_token,
                                                                                                word2index)
            path_indexes.append(path_indexed)
            path_statement_lens.append(path_statement_len)
            path_token_lens.append(path_token_len)
            if len(path_indexes) >= len_path:
                break
        if len(path_indexes) < len_path:
            for i in range(0, len_path - len(path_indexes)):
                path_indexed, path_statement_len, path_token_len = path_statements_to_indexes([],
                                                                                                    len_path_statement,
                                                                                                    len_path_statement_token,
                                                                                                    word2index)
                path_indexes.append(path_indexed)
                path_statement_lens.append(path_statement_len)
                path_token_lens.append(path_token_len)
        path_indexes = path_indexes[:len_path]
        path_token_lens = path_token_lens[:len_path]
        real_len = min(len(paths), len_path)
        path_indexes_batch.append(path_indexes)
        path_len_batch.append([max(real_len, 1)])
        path_statement_len_batch.append(path_statement_lens)
        path_token_len_batch.append(path_token_lens)
    return torch.from_numpy(np.array(path_indexes_batch)).to(config.device), torch.from_numpy(np.array(path_len_batch)).to(config.device),torch.from_numpy(np.array(path_statement_len_batch)).to(config.device), torch.from_numpy(np.array(path_token_len_batch)).to(config.device)
    # return np.array(path_indexes), np.array([max(real_len, 1)]), np.array(path_statement_lens), np.array(path_token_lens)

def path_statements_to_indexes(path, len_path_statement, len_path_statement_token, word2index):
    statement_indexes = []
    statement_lens = []
    for statement in path:
        statement_indexed, statement_len = sequence_to_indexes(statement, len_path_statement_token,
                                                                    word2index)
        statement_indexes.append(statement_indexed)
        statement_lens.append(statement_len)
        if len(statement_indexes) >= len_path_statement:
            break
    if len(statement_indexes) < len_path_statement:
        for i in range(0, len_path_statement - len(statement_indexes)):
            statement_indexed, statement_len = sequence_to_indexes("", len_path_statement_token,
                                                                        word2index)
            statement_indexes.append(statement_indexed)
            statement_lens.append(statement_len)
    statement_indexes = statement_indexes[:len_path_statement]
    statement_lens = statement_lens[:len_path_statement]
    real_len = min(len(path), len_path_statement)
    return np.array(statement_indexes), np.array([max(1, real_len)]), np.array(statement_lens)

def sequence_to_indexes(sequence, length, word2index):
    indexes = np.zeros(length, dtype=np.int64)
    # token = sequence.strip().lower().split(' ')
    token = sequence.strip().split(' ')
    length = min(len(token), length)
    for i in range(length):
        indexes[i] = word2index.get(token[i], word2index[_UNK])

    return indexes, np.array([max(length, 1)])

def get_path_max_len(paths):
    lens = []
    max_len = 0
    for path in paths:
