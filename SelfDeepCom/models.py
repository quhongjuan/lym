...
from PathTokenEncoder import PathTokenEncoder
from PathStatementEncoder import PathStatementEncoder
from PathAttention import PathAttention
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, seq_lens: torch.Tensor):
        ...
        # 下面是想利用正反向的信息，但是不太清楚正向t 和反向t对应还是和len-t对应
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, last_hidden: torch.Tensor,
        ...
        code_attn_weights = self.code_attention(last_hidden, code_outputs)  # [1, B, H] [T, B, H] --> [B, 1, T]
        code_context = code_attn_weights.bmm(code_outputs.transpose(0, 1))  # [B, 1, T] * [B, T, H] -->[B, 1, H]
        ...
        context = code_context + ast_context + path_vector    # [1, B, H]
...
class Model(nn.Module):
    ...
    def __init__(self, code_vocab_size, ast_vocab_size, nl_vocab_size,
        ...
        self.path_token_encoder = PathTokenEncoder(self.code_vocab_size, config.embedding_dim, config.hidden_size)
        self.path_token_attention = PathAttention(2*config.hidden_size, config.hidden_size)
        self.path_statement_encoder = PathStatementEncoder(2*config.hidden_size, config.hidden_size)
        self.path_statement_attention = PathAttention(2*config.hidden_size, config.hidden_size)
        self.path_attention = PathAttention(2*config.hidden_size, config.hidden_size)
        self.path_linear = nn.Linear(2 * config.hidden_size, config.hidden_size)
        self.reduce_hidden = ReduceHidden()
        self.decoder = Decoder(nl_vocab_size)


        if config.use_cuda:
            self.code_encoder = self.code_encoder.to(config.device)
            self.ast_encoder = self.ast_encoder.to(config.device)
            self.path_token_encoder = self.path_token_encoder.to(config.device)
            self.path_token_attention = self.path_token_attention.to(config.device)
            self.path_statement_encoder = self.path_statement_encoder.to(config.device)
            self.path_statement_attention = self.path_statement_attention.to(config.device)
            self.path_attention = self.path_attention.to(config.device)
            self.path_linear = self.path_linear.to(config.device)
            self.reduce_hidden = self.reduce_hidden.to(config.device)
        ...
        if is_eval:
            ...
            self.path_token_encoder.eval()
            self.path_token_attention.eval()
            self.path_statement_encoder.eval()
            self.path_statement_attention.eval()
            self.path_attention.eval()
            self.path_linear.eval()
    ...
    def forward(self, batch, batch_size, nl_vocab, is_test=False):
        ...
        """
        # batch: [T, B]
        code_batch, code_seq_lens, ast_batch, ast_seq_lens, nl_batch, nl_seq_lens, path_batch, path_real_lens, path_statement_real_lens, path_token_real_lens= batch
        ...
        path_batch_size = path_batch.size(0)
        path_batch_num = path_batch.size(1)
        statement_num = path_batch.size(2)
        path_batch = path_batch.view(path_batch_size*path_batch_num*statement_num, -1)
        path_token_real_lens = path_token_real_lens.view(path_batch_size*path_batch_num*statement_num, -1)

        path_vector = self.path_token_encoder(path_batch, path_token_real_lens)
        path_vector = self.path_token_attention(path_vector)

        path_vector = path_vector.view(path_batch_size*path_batch_num, statement_num, -1)
        path_statement_real_lens = path_statement_real_lens.view(path_batch_size*path_batch_num, -1)

        path_vector = self.path_statement_encoder(path_vector, path_statement_real_lens)
        path_vector = self.path_statement_attention(path_vector)

        path_vector = path_vector.view(path_batch_size, path_batch_num, -1)
        path_vector = self.path_attention(path_vector)

        path_vector = self.path_linear(path_vector)
        path_vector = path_vector.unsqueeze(0)

        # data for decoder
        code_hidden = code_hidden[:1]  # [1, B, H]
        ast_hidden = ast_hidden[:1]  # [1, B, H]
        # 下面是想利用正反向的信息，但是不太清楚正向t 和反向t对应还是和len-t对应
        # code_hidden = 1/2(code_hidden[:1] + code_hidden[1:]) # [1, B, H]
        # ast_hidden = 1/2(ast_hidden[:1] + ast_hidden[1:]) # [1, B, H]
        # 正解： 双向RNN中，outputs 处在中间位置，有考虑前后的词的意义
        # hidden作为最后一个 是包含了所有词的信息，所以没有考虑双向的必要？
        decoder_hidden = self.reduce_hidden(code_hidden, ast_hidden)  # [1, B, H]

        if is_test:
            return code_outputs, ast_outputs, decoder_hidden, path_vector
    ...
    def set_state_dict(self, state_dict):
        ...
        self.path_token_encoder.load_state_dict(state_dict["path_token_encoder"])
        self.path_token_attention.load_state_dict(state_dict["path_token_attention"])
        self.path_statement_encoder.load_state_dict(state_dict["path_statement_encoder"])
        self.path_statement_attention.load_state_dict(state_dict["path_statement_attention"])
        self.path_attention.load_state_dict(state_dict["path_attention"])
        self.path_linear.load_state_dict(state_dict["path_linear"])
